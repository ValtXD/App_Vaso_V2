...
import 'chat_page.dart'; // 🔹 import do chat
...
class HomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final moistureOk =
        moisture >= sensor.moistureLow && moisture <= sensor.moistureHigh;
    final lightOk =
        light >= sensor.lightLow && light <= sensor.lightHigh;
            ...
            Row(
              ...
              children: [
                ElevatedButton.icon(
                  ...
                  icon: Icon(sensor.pumpOn
                      ? Icons.water_drop
                      : Icons.water_drop_outlined),
                  label: Text(sensor.pumpOn
                      ? 'Bomba ligada 💧'
                      : 'Ligar bomba'),
                  style: ElevatedButton.styleFrom(
                    backgroundColor:
                    sensor.pumpOn ? Colors.blue : Colors.grey.shade300,
                    foregroundColor:
                    sensor.pumpOn ? Colors.white : Colors.black87,
                  ),
                ),
                ElevatedButton.icon(
                  onPressed: () => sensor.toggleLamp(),
                  icon: Icon(sensor.lampOn
                      ? Icons.lightbulb
                      : Icons.lightbulb_outline),
                  label: Text(sensor.lampOn
                      ? 'Lâmpada ON'
                      : 'Ligar lâmpada'),
                  style: ElevatedButton.styleFrom(
                    backgroundColor:
                    sensor.lampOn ? Colors.amber : Colors.grey.shade300,
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: [
        ...
        Text('Tema inspirado em plantas',
