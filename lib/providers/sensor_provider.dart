...
class SensorProvider extends ChangeNotifier {
  ...
  Future<void> _loadHistory() async {
    ...
    if (data != null) {
      ...
      _history
        ..clear()
        ..addAll(list.map((e) => SensorReading.fromJson(e as Map<String, dynamic>)));
      notifyListeners();
    }
  }
  ...
  void _saveHistoryIfNeeded() async {
    final now = DateTime.now();
    if (_lastSaved == null || now.difference(_lastSaved!).inMinutes >= 1) {
  ...
  void togglePump() {
    pumpOn = !pumpOn;
    _service.setPump(pumpOn);
    notifyListeners();
  }

  void toggleLamp() {
    lampOn = !lampOn;
    _service.setLamp(lampOn);
    notifyListeners();
  }
